...
/**
 * פתרון ה-404 הסופי:
 * אנחנו מגדירים את ה-apiVersion כ-'v1' בתוך הקריאה למודל.
 */
...
export const getMentorAdvice = async (stage: any, plan: any) => {
  ...
  try {
    const result = await model.generateContent(`אתה מנטור אסטרטגי לשפ"ח. שלב: ${stage}. תן עצה קצרה בעברית ב-JSON: {"content": "..."}`);
...
export const integrateFullPlanWithAI = async (plan: any) => {
  ...
  try {
    const result = await model.generateContent(`שפר תוכנית עבודה לשפ"ח: ${JSON.stringify(plan)}. החזר JSON.`);
    const response = await result.response;
